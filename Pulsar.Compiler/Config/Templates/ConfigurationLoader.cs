    ...
    internal static class ConfigurationLoader
    {
        ...
        internal static RuntimeConfig LoadConfiguration(string[] args, bool requireSensors = true, string? configPath = null)
        {
            try
            {
                ...
                {
                    _logger.Warning("No configuration file found at {Path}, using defaults", configPath ?? "default location");
                }
        ...
        private static void ValidateConfiguration(RuntimeConfig config, bool requireSensors)
        {
            ...
            if (requireSensors && (config.RequiredSensors == null || config.RequiredSensors.Length == 0))
            {
                ...
                throw new InvalidOperationException("Required sensors must be configured when requireSensors is true");
            }

            if (config.CycleTime.HasValue && config.CycleTime.Value < TimeSpan.FromMilliseconds(10))
            {
                _logger.Warning("Cycle time is very low ({CycleTime}ms), this may impact performance", 
