    ...
    public class BeaconBuildOrchestratorFixed
    {
        ...
        public async Task<BuildResult> BuildBeaconAsync(BuildConfig config)
        {
            try
            {
                ...
                bool createSeparateDir = true; // Hard-coded for now, normally would use config.CreateSeparateDirectory
                if (createSeparateDir)
                {
                    beaconOutputDir = Path.Combine(outputDir, "Beacon");
                ...
                var solutionPath = Path.Combine(beaconOutputDir, "Beacon.sln");
                ...
                bool generateTestProject = true; // Hard-coded for now, normally would use config.GenerateTestProject
