...
public class Program
{
    ...
    private static async Task<int> GenerateBuildableProject(
    ...
    )
    {
        ...
        try
        {
            ...
            if (result.Success)
            {
                ...
                logger.Information("To build the runtime, run:");
                logger.Information(
                    "  cd {OutputPath}",
                    options.GetValueOrDefault("output", "Generated")
                );
                logger.Information(
    ...
    private static async Task<int> InitializeProject(
        Dictionary<string, string> options,
        ILogger logger
    )
    {
