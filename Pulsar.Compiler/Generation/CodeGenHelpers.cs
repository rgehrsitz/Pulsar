...
namespace Pulsar.Compiler.Generation
{
    ...
    public static class CodeGenHelpers
    {
        ...
        public static string GenerateAOTAttributes(string namespace1)
        {
            ...
            sb.AppendLine(
                "[assembly: DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(StackExchange.Redis.ConnectionMultiplexer))]"
            );
            sb.AppendLine(
                "[assembly: DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(StackExchange.Redis.IDatabase))]"
            );
            sb.AppendLine(
                "[assembly: DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(StackExchange.Redis.HashEntry))]"
            );

            // Add attributes to preserve buffer types for temporal rules
            sb.AppendLine(
                $"[assembly: DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof({namespace1}.Buffers.RingBufferManager))]"
            );
            sb.AppendLine(
                $"[assembly: DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof({namespace1}.Buffers.CircularBuffer))]"
            );

            // Add attributes to preserve rule interfaces
            sb.AppendLine(
                $"[assembly: DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof({namespace1}.Interfaces.IRuleCoordinator))]"
            );
            sb.AppendLine(
                $"[assembly: DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof({namespace1}.Interfaces.IRuleGroup))]"
            );

            // Add attributes to preserve metrics types
            sb.AppendLine(
                "[assembly: DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(Prometheus.Metrics))]"
            );
            sb.AppendLine(
                "[assembly: DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(Prometheus.Counter))]"
            );
            sb.AppendLine(
                "[assembly: DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(Prometheus.Histogram))]"
            );

            // Add attributes to preserve service types
            sb.AppendLine(
                $"[assembly: DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof({namespace1}.Services.RedisService))]"
            );
        ...
        )
        {
            ...
            {
                _logger.Debug(
                    "Generating condition code for {Count} conditions",
                    conditions?.All?.Count ?? 0
                );
        ...
        public static string GenerateActionCode(
        ...
        {
            _logger.Debug(
                "Generating expression for condition type: {Type}",
                condition.GetType().Name
            );

            // Default implementation - should be overridden for specific condition types
            throw new NotImplementedException(
                $"Generation of condition type {condition.GetType().Name} is not implemented"
            );
        }
        ...
        private static string GenerateActionExpression(ActionDefinition action)
        {
            ...
            throw new NotImplementedException(
                $"Generation of action type {action.GetType().Name} is not implemented"
            );
        }
