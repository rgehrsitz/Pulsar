// File: Pulsar.Compiler/Generation/CodeGeneratorFixed.cs
...
{
    public class CodeGeneratorFixed : IDisposable
    {
        private readonly ILogger<CodeGeneratorFixed> _logger;
        private readonly RuleGroupGeneratorFixed _ruleGroupGenerator;
        private readonly RuleCoordinatorGenerator _ruleCoordinatorGenerator;
        private readonly MetadataGenerator _metadataGenerator;

        public CodeGeneratorFixed(ILogger<CodeGeneratorFixed>? logger = null)
        {
            _logger = logger ?? NullLogger<CodeGeneratorFixed>.Instance;
            _ruleGroupGenerator = new RuleGroupGeneratorFixed(_logger);
        ...
        )
        {
            ...
            // Generate rule groups using the fixed generator
        ...
        public GeneratedFileInfo GenerateEmbeddedConfig(BuildConfig buildConfig)
        {
            ...
            content += "    {" + Environment.NewLine;
            content += "        public const string SystemConfigJson = \"{}\";" + Environment.NewLine;
            content += "    }" + Environment.NewLine;
            content += "}" + Environment.NewLine;
