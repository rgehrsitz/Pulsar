...
namespace Pulsar.TestAutomation
{
    ...
    public class PulsarBeaconTester : IDisposable
    {
        ...
        /// </summary>
        public async Task<bool> RunEndToEndTests(string rulesPath, string configPath, string scenariosPath)
        {
            try
            {
        ...
        private async Task<bool> CompileRules(string rulesPath, string configPath)
        {
            ...
            var process = new Process
            {
                ...
                {
                    FileName = "dotnet",
                    Arguments = $"\"{compilerPath}\" beacon --rules=\"{rulesPath}\" --config=\"{configPath}\" --output=\"{outputDir}\" --verbose",
            ...
            _logger.LogInformation("Running Pulsar compiler: {Command} {Args}", 
            ...
            {
                _logger.LogError("Pulsar compilation failed with exit code: {ExitCode}", process.ExitCode);
                return false;
            }
            
            _logger.LogInformation("Pulsar compilation completed successfully. Output directory: {Dir}", outputDir);
            return true;
        }
        ...
        private async Task<bool> StartBeacon()
        {
            ...
            {
                _logger.LogError("Beacon process exited prematurely with code: {Code}", _beaconProcess.ExitCode);
                return false;
            }
        ...
        private List<TestScenario> LoadTestScenarios(string scenariosPath)
        {
            try
            {
                ...
                var scenarios = JsonSerializer.Deserialize<TestScenarios>(json, 
        ...
        {
            _logger.LogInformation("Running scenario: {Name} - {Description}", scenario.Name, scenario.Description);
                ...
                foreach (var expected in scenario.ExpectedOutputs)
                {
                    ...
                    if (expectedValue is bool expectedBool)
                    {
                        ...
                        else
                        {
                            ...
                            var actualBoolValue = lowerValue == "true" || lowerValue == "1" || lowerValue == "yes";
                            comparisonResult = (actualBoolValue == expectedBool);
                        }
                    }
                    else if (expectedValue is double || expectedValue is int || expectedValue is float || expectedValue is decimal)
                    {
                        // Numeric comparison with tolerance
                        var expectedNum = Convert.ToDouble(expectedValue);
                        if (double.TryParse(actualValueStr, out var actualNum))
                        {
                            comparisonResult = Math.Abs(actualNum - expectedNum) <= scenario.Tolerance;
                        }
                    }
                    else
                    {
                    ...
                    if (comparisonResult)
                    {
                        _logger.LogInformation("Output {Key} = {Value} matches expected {Expected}",
                            key, actualValueStr, expectedValue);
                    }
                    else
                    {
                        _logger.LogError("Output {Key} = {Value} does not match expected {Expected}",
                            key, actualValueStr, expectedValue);
                        passed = false;
                    }
                }
        ...
        private async Task ClearRedisData()
        {
            try
            {
                ...
                var keys = server.Keys(pattern: "input:*").Union(
                           server.Keys(pattern: "output:*")).Union(
                           server.Keys(pattern: "buffer:*")).ToArray();
  ...
  ""CycleTimeMs"": 100
        ...
        private string FindCompilerPath()
        {
            var searchPaths = new[]
            {
                ...
                Path.Combine(Directory.GetCurrentDirectory(), "..", "Pulsar.Compiler", "bin", "Debug", "net9.0", "Pulsar.Compiler.dll"),
            ...
            {
                var found = Directory.GetFiles(rootDir, "Pulsar.Compiler.dll", SearchOption.AllDirectories);
                if (found.Length > 0)
                {
                    return found[0];
                }
        ...
        private string FindBeaconRuntimePath(string beaconDir)
        {
            ...
            {
                // Main project binary
                Path.Combine(beaconDir, "Beacon.Runtime", "bin", "Debug", "net9.0", "Beacon.Runtime.dll"),
                Path.Combine(beaconDir, "Beacon.Runtime", "bin", "Release", "net9.0", "Beacon.Runtime.dll"),
                // Published binary
                Path.Combine(beaconDir, "Beacon.Runtime", "bin", "Debug", "net9.0", "publish", "Beacon.Runtime.dll"),
                Path.Combine(beaconDir, "Beacon.Runtime", "bin", "Release", "net9.0", "publish", "Beacon.Runtime.dll"),
            };
            ...
            // If not found in expected places, search recursively
            var found = Directory.GetFiles(beaconDir, "Beacon.Runtime.dll", SearchOption.AllDirectories);
            if (found.Length > 0)
            {
                return found[0];
            }
    ...
    public class PulsarBeaconEndToEndTests
    {
        ...
        {
            var workingDir = Path.GetFullPath(Path.Combine(Directory.GetCurrentDirectory(), "TestAutomation"));
