...
namespace Pulsar.Tests.Integration
{
    public class IntegrationTests
    {
        ...
        public void Integration_EndToEnd_Succeeds()
        {
            ...
            string[] ruleContents = new string[] { "valid rule content", "another valid rule content" };
        ...
        public void Integration_LoggingAndMetrics_Succeeds()
        {
            ...
            var logs = RuntimeEngine.RunCycleWithLogging(new string[] { "valid rule content" }, new Dictionary<string, string> { { "SensorA", "123" } });
            
            // Assert: Validate that logs contain enhanced metrics messages using substring matching
            Assert.Contains("Cycle Started", logs);
            Assert.True(logs.Any(log => log.Contains("Processing rules:")), "Expected at least one log entry to contain 'Processing rules:'");
            Assert.True(logs.Any(log => log.Contains("Processed Rules:")), "Expected at least one log entry to contain 'Processed Rules:'");
            Assert.True(logs.Any(log => log.Contains("Cycle Duration:")), "Expected at least one log entry to contain 'Cycle Duration:'");
        ...
        public void Integration_MultipleRulesAndSensors_Succeeds()
        {
            ...
            string[] ruleContents = new string[] { "valid rule content 1", "valid rule content 2", "valid rule content 3" };
            ...
            Assert.Contains("Cycle Started", logs);
            Assert.True(logs.Any(log => log.Contains("Processing rules:")), "Expected log for processing rules.");
            Assert.True(logs.Any(log => log.Contains("Processed Rules:")), "Expected log for processed rules.");
            Assert.True(logs.Any(log => log.Contains("Cycle Duration:")), "Expected log for cycle duration.");
            Assert.Contains("Cycle Ended", logs);
            Assert.True(output.ContainsKey("result") && output["result"] == "success", "Expected runtime output to indicate success.");
        }
        ...
        public void Integration_StressTest_LargeRuleSet()
        {
            ...
            var ruleContents = Enumerable.Range(1, 1000).Select(i => $"valid rule content {i}").ToArray();
            ...
            Assert.Contains("Cycle Started", logs);
            Assert.True(logs.Any(log => log.Contains("Processing rules:")), "Expected log entry for processing rules.");
            Assert.True(logs.Any(log => log.Contains("Processed Rules:")), "Expected log entry for processed rules.");
            Assert.True(logs.Any(log => log.Contains("Cycle Duration:")), "Expected log entry for cycle duration.");
            Assert.Contains("Cycle Ended", logs);
            Assert.True(output.ContainsKey("result") && output["result"] == "success", "Expected runtime output to indicate success.");
        }
    }
    ...
    {
        public static Dictionary<string, string> RunCycle(string[] rules, Dictionary<string, string> simulatedSensorInput)
        {
        ...
        public static List<string> RunCycleWithLogging(string[] rules, Dictionary<string, string> simulatedSensorInput)
        {
