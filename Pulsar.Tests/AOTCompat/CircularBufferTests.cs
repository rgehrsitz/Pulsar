    ...
    public class CircularBufferTests
    {
        ...
        private class SimpleCircularBuffer<T>
        {
            ...
            public double Average()
            {
                ...
                for (int i = 0; i < _count; i++)
                {
                    ...
                    {
                        throw new InvalidOperationException($"Cannot convert value at index {i} to double for average calculation", ex);
                    }
                }
                return sum / _count;
            }
            ...
            public double Max()
            {
                ...
                for (int i = 0; i < _count; i++)
                {
                    ...
                    {
                        throw new InvalidOperationException($"Cannot convert value at index {i} to double for max calculation", ex);
                    }
                }
            ...
            public double Min()
            {
                ...
                for (int i = 0; i < _count; i++)
                {
                    ...
                    {
                        throw new InvalidOperationException($"Cannot convert value at index {i} to double for min calculation", ex);
                    }
                }
        ...
        private class TimestampedCircularBuffer<T>
        {
            ...
            public bool IsIncreasingInTimespan(TimeSpan timespan)
            {
                ...
                if (recentValues.Count < 2) return true;
                
                for (int i = 0; i < recentValues.Count - 1; i++)
                {
                    if (Convert.ToDouble(recentValues[i].Value) > Convert.ToDouble(recentValues[i + 1].Value))
                        return false;
                }
