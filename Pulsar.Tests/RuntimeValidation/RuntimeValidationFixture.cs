...
namespace Pulsar.Tests.RuntimeValidation
{
    ...
    public class RuntimeValidationFixture : IAsyncLifetime, IDisposable
    {
        ...
        public RuntimeValidationFixture()
        {
            ...
            _testOutputPath = Path.Combine(Directory.GetCurrentDirectory(), "RuntimeValidation");
...
";
        ...
        public async Task<bool> BuildTestProject(string[] ruleFiles)
        {
            ...
            var compileResult = compiler.Compile(
                rules.ToArray(),
                new CompilerOptions { BuildConfig = buildConfig }
            );
            
            if (!compileResult.Success)
            {
                _logger.LogError("Compilation failed: {Errors}", 
                    string.Join(Environment.NewLine, compileResult.Errors.Select(e => e.ToString())));
                return false;
            }
        ...
        /// <summary>
        /// Builds the generated project and loads the assembly for testing
        /// </summary>
        private async Task<bool> BuildProject()
        {
            // Create additional files needed for compilation
            ...
            {
                FileName = "dotnet",
                Arguments = $"build {Path.Combine(_testOutputPath, "RuntimeTest.csproj")} --configuration Debug",
            ...
            try
            {
                var assemblyPath = Path.Combine(_testOutputPath, "bin", "Debug", "net9.0", "RuntimeTest.dll");
                _compiledAssembly = Assembly.LoadFrom(assemblyPath);
                return true;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to load compiled assembly");
                return false;
            }
        }
