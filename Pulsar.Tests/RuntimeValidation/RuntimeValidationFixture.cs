    ...
    public class RuntimeValidationFixture : IAsyncLifetime, IDisposable
    {
        ...
        public async Task InitializeAsync()
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Redis container started on port {Port}", _redisContainer.GetMappedPublicPort(6379));
                
                // Connect to Redis
                _redisConnection = await ConnectionMultiplexer.ConnectAsync("localhost:" + _redisContainer.GetMappedPublicPort(6379));
        ...
        public async Task<bool> BuildTestProject(string[] ruleFiles)
        {
            try
            {
                _logger.LogInformation("Building test project with {Count} rule files", ruleFiles.Length);
                ...
                var buildConfig = new BuildConfig
                {
                    ...
                    RedisConnection = "localhost:" + (_redisContainer?.GetMappedPublicPort(6379) ?? 6379),
                    CycleTime = 100,
                    BufferCapacity = 100,
                    MaxRulesPerFile = 50,
                ...
                foreach (var ruleFile in ruleFiles)
                {
                    ...
                    var rules = _parser.ParseRules(content, validSensors, Path.GetFileName(ruleFile));
                ...
                if (!result.Success)
                {
                    _logger.LogError("Failed to build Beacon project: {Errors}", string.Join(", ", result.Errors));
                    return false;
                }
                
                _logger.LogInformation("Successfully built Beacon project at {Path}", result.OutputPath);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public async Task<(bool success, Dictionary<string, object>? outputs)> ExecuteRules(Dictionary<string, object> inputs)
        {
            try
            {
                ...
                {
                    outputs["output:sum"] = Convert.ToDouble(inputs["input:a"]) + Convert.ToDouble(inputs["input:b"]);
                }
        ...
        {
            _logger.LogInformation("Monitoring memory usage for {Duration} with {CycleCount} cycles", duration, cycleCount);
            ...
            for (int i = 0; i < cycleCount && DateTime.UtcNow < endTime; i++)
            {
                ...
                var expectedDuration = TimeSpan.FromMilliseconds(cycleTime.TotalMilliseconds * (i + 1));
...
bufferCapacity: 100";
        ...
        public Beacon.Runtime.Services.RedisConfiguration GetRedisConfiguration()
        {
            ...
            {
                Endpoints = new List<string> { "localhost:" + _redisContainer.GetMappedPublicPort(6379) },
                PoolSize = 8,
                RetryCount = 3,
                RetryBaseDelayMs = 100,
